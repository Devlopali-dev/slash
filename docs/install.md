...
```bash
docker run -d --name slash --publish 5231:5231 --volume ~/.slash/:/var/opt/slash devlopali/slash:latest
```
...
```bash
docker pull devlopali/slash:latest
```
...
curl -#LO https://github.com/devlopali-dev/slash/raw/main/docker-compose.yml
docker compose up -d
```
...
```shell
docker run -d --name slash --publish 5231:5231 --volume ~/.slash/:/var/opt/slash devlopali/slash:latest --driver postgres --dsn 'postgresql://postgres:PASSWORD@localhost:5432/slash'
```
